...
fid = fopen('THERM.dat', 'r' );


%% File Opening Check
if fid == -1
disp('thermo.dat File open not successful')
error('THERM.dat not found')
...
for i=1:n(2)
    count=0;
    for j=1:m(2)
        if strcmpi(spcs(i).species,ttdata(j).name)==1
    ...
    if count==0
        ...
        error('Species not found. Check for above species in thermo.dat file and chemical mechanism file');
    end
end
    ...
    for i=1:n(2)
        for j=1:p(2)
            ...
            for k=1:4 
                ...
                if strcmpi(elements(j).elements,elements_th)==1   
