...
unbalanced_prod_matrix,rev,third_body,third_body_matrix,reaction_order_matrix,reverse_reaction_order_matrix,global_reactants,...
...
fid = fopen('hydrogencode.inp');


%% File Opening Check
if fid == -1
    disp('inp File open not successful')
else
    disp('inp File open successful')
...
for i=1:no_of_elements(2)  %no. of elements in input file
    for j=1:p(1) %total no. of elements on mass_table file
        ...
        if strcmp(temp_data,elm(i).elements)==1 %%compare elements
